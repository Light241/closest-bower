...
var lowest  = require.resolve('./fixtures/a/b/bower.json');
var local   = require.resolve('./bower.json');

test('async: always true', function(t) {
  t.plan(1);

  closest(__dirname, function() {
    return true
  }, function(err, result) {
    if (err) return t.fail(err);
    t.ok(result === local, 'found local bower.json');
  })
});

test('sync: always true', function(t) {
  var result = closest.sync(__dirname, function() {
    return true
  });
...
test('async: three levels down', function(t) {
  ...
  closest(path.dirname(lowest), localDir, function(err, result) {
      if (err) return t.fail(err);
...
test('async: three levels down', function(t) {
  ...
  t.ok(result, 'should find bower.json');
  t.equal(3, require(result).level, '"level": 3');
  t.end()
});
